// icemaster.c -- 天師派素衣一脈真人 白齊（號素衣真人，太乙觀觀主，冰道師父）
...
void create()
{
	set_name("白齊", ({ "bai qi", "frost priest", "suyi", "master", "icemaster" }));
	set("nickname", "素衣真人");
	set_attr("str", 20);
	set_attr("dex", 22);
	set_attr("int", 26);
	...
	set("age", 54);
	set("long",
		"這位便是天師派素衣一脈的真人白齊﹐一身素白道袍纖塵不染﹐頭戴蓮\n"
		"花冠﹐手執一柄玉柄拂塵﹐面如冠玉﹐雙目清冷如水﹐周身隱隱透著一\n"
		"股沁人的寒氣。天師派素衣一脈以冰證道﹐白齊浸淫冰術數十年﹐\n"
...
int accept_apprentice(object me)
{
	...
	if( me->query_class() != "commoner" ) {
		do_chat("白齊輕拂拂塵﹐說道﹕你既已入別派﹐道緣已定﹐何苦再來﹖\n");
		return 0;
	}

	// 須先在江湖上歷練過方可入道
	if( me->query_level() < 1 ) {
		do_chat("白齊說道﹕你塵緣未了﹐且去歷練一番再來罷。\n");
		return 0;
	}

	// 冰術全憑靈性引動﹐靈性過低者難窺門徑（門檻甚低﹐尋常人皆可入）
	if( me->query_attr("spi") < 14 ) {
		do_chat("白齊端詳了你一會﹐搖頭道﹕你靈性閉塞﹐與冰無緣﹐難學我這路道法。\n");
		return 0;
	}

	do_chat(({
		"白齊上下打量了你一番﹐微微頷首。\n",
		"白齊說道﹕我天師派素衣一脈﹐以冰證道﹐你可有這份道心﹖\n",
		"白齊說道﹕也罷﹐你既與冰有緣﹐我便收你入素衣門下﹐傳你天師道法。\n",
	}));
	return 1;
}
...
int init_apprentice(object me)
{
	...
	if( ::init_apprentice(me) ) {
		...
		message_vision(
			...
			this_object(), me);
		do_chat(({
			"白齊說道﹕從今日起﹐你便是我天師派素衣門下的弟子了。\n",
			"白齊說道﹕道法無涯﹐你且到素心軒對冰參修﹐莫負了這身道法。\n",
		}));
	}
}
