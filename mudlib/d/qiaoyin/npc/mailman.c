...
int do_ask(string arg)
{
	...
	if( q >= 1 ) {
		...
		do_chat(({
			(: command, "say 客官那封桐城密信可還收著？好。信中所指的舊蹤﹐須得客官親自循著一處處訪下去——莫嫌路遠機關重重﹐桐城派失落的真正信物﹐便埋在那條老路的盡頭。" :),
	...
	do_chat(({
		...
		(: command, "say 看客官是個有心肯奔走的﹐這封密信﹐老漢便託付與你了。自喬陰起步﹐循線追查下去罷——這只是一樁長路的開端。珍重﹐老漢就在這大街上等客官的好消息。" :),
	}));
	return 1;
}
