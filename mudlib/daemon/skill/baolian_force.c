...
//
// 這是一門內功(force)心法﹐寶蓮寺以慈悲清淨為本的根本吐納之術。內功與
// 招式 daemon (buddha_zarn.c) 不同﹐它是被動的「力道乘數」﹕戰鬥時
...
//
// 玩家透過 map_skill("force","baolian force") 將內功對應到本心法後﹐力道
// 才會隨內功修為成長。修煉內功的指令是 exert（見 cmds/std/exert.c）﹐它
// 會呼叫本 daemon 的 exert_function()。
//
// 本檔比照白象寺的蓮華心法 daemon/skill/lotus_force.c 複製。寶蓮心法在
// 中文字典尚無詞條（白象用的 lotus force=蓮華心法另有其詞）﹐故比照大內
// 罡氣 danei_force.c 的作法﹐於 create() 時以 CHINESE_D->add_translate()
// 自行登錄中文名「寶蓮心法」。
//
...
// 寶蓮寺弟子打坐運功時所現的內功氣象（純文字氛圍﹐無戰鬥數值）。
string *exert_msg = ({
  "$N閉目盤膝﹐緩緩運轉寶蓮心法﹐周身似有一股清淨慈和之氣氤氳流轉。\n",
  "$N凝神吐納﹐丹田之中一朵寶蓮悄然綻放﹐祥和之意循經滋長。\n",
  "$N默運心法﹐但覺真氣清明圓融﹐慈悲之中自有一股化解魔障的清淨之力。\n",
});
