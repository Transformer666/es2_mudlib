...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("qidaoforce");
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("qidaoforce", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐奇道內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("qidaoforce", 40);
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("spi") + me->query_attr("int") + me->query_skill("force", 1)) > 20 ) {
        me->damage_stat("gin", 1);
        me->damage_stat("kee", 1);
        me->damage_stat("sen", 1);
        me->improve_skill("qidaoforce", random(me->query_attr("spi")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "qidaoforce":
        message_vision(HIY "$N盤膝而坐﹐運起奇道內功心法﹐周身奇氣流轉。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("奇道內功沒有這種功能。\n");
    }
}
