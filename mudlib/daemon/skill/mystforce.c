...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("mystforce");
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("mystforce", 1);
    learn = me->query_learn("mystforce", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和，步玄心法的內功已經練成了！\n" NOR);
            me->advance_skill("mystforce", 40);
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("int") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        ...
        me->improve_skill("mystforce", random(me->query_attr("int")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "mystforce":
        message_vision(HIY "$N盤膝而坐，運起步玄心法。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("步玄心法沒有這種功能。\n");
    }
}
