...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("taoism");
    setup();
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("taoism");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺你感覺自己對道法的領悟更深了一層﹗\n" NOR);
            me->advance_skill("taoism", 40);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("wis") < level / 3 ) {
        tell_object(me, HIW "由於你道法的修練有成﹐你的悟性提高了。\n" NOR);
        me->add("wis", 1);
    }
}
