...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("snowshade sword");
    setup();
}
...
attack_using(object me, object opponent, object weapon)
{
    ...
    damage = COMBAT_D->fight(me, opponent, "snowshade sword", actions[random(sizeof(actions))], weapon);
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("snowshade sword");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺雪影劍法的招式已經融會貫通﹗\n" NOR);
            me->advance_skill("snowshade sword", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("snowshade sword", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("dex") < level / 3 ) {
        tell_object(me, HIW "由於你雪影劍法的修練有成﹐你的敏捷提高了。\n" NOR);
        me->add("dex", 1);
    }
}
