...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("snowshade force");
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("snowshade force", 1);
    learn = me->query_learn("snowshade force", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐雪影內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("snowshade force", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("snowshade force", 1);
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("spi") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        ...
        me->improve_skill("snowshade force", random(me->query_attr("spi")/10) + 1);
...
{
    switch(func) {
    case "snowshade force":
        message_vision(HIY "$N盤膝而坐﹐運起雪影內功﹐寒氣如雪般飄散。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("雪影內功沒有這種功能。\n");
    }
}
