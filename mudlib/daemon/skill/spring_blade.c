...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("spring blade");
    setup();
}
...
attack_using(object me, object opponent, object weapon)
{
    ...
    damage = COMBAT_D->fight(me, opponent, "spring blade", actions[random(sizeof(actions))], weapon);
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("spring blade", 1);
    learn = me->query_learn("spring blade");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺泉刀法的招式已經融會貫通﹗\n" NOR);
            me->advance_skill("spring blade", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("spring blade", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("str") < level / 3 ) {
        tell_object(me, HIW "由於你泉刀法的修練有成﹐你的臂力提高了。\n" NOR);
        me->add("str", 1);
    }
}
