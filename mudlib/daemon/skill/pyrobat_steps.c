...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("pyrobat steps");
    setup();
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("pyrobat steps");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺身形如火蝠般敏捷﹐火蝠步已經練成了﹗\n" NOR);
            me->advance_skill("pyrobat steps", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("pyrobat steps", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("dex") < level / 3 ) {
        tell_object(me, HIW "由於你火蝠步的修練有成﹐你的敏捷提高了。\n" NOR);
        me->add("dex", 1);
    }
}
