...
//
// 這是一門內功(force)心法﹐天師派玄衣一脈以風證道的根本吐納之術。內功
// 與招式 daemon (taoism-wind.c) 不同﹐它是被動的「力道乘數」﹕戰鬥時
...
//
// 玩家透過 map_skill("force","galewind force") 將內功對應到本心法
// 後﹐力道才會隨內功修為成長。修煉內功的指令是 exert（見 cmds/std/
// exert.c）﹐它會呼叫本 daemon 的 exert_function()。
//
// 本檔比照天師派素衣 daemon/skill/polar-silkworm_force.c 複製﹐將寒氣易為罡風。
// 注意 key 含空白(galewind force)﹐故檔名為
// galewind-force.c（空白換連字號）。
...
// 天師派玄衣弟子打坐運功時所現的內功氣象（純文字氛圍﹐無戰鬥數值）。
string *exert_msg = ({
  "$N閉目盤膝﹐緩緩運轉玄風訣﹐周身似有一股清靈的罡風盤旋不散。\n",
  "$N凝神吐納﹐丹田之中一團罡氣冉冉升起﹐風意循經而行﹐生生不息。\n",
  "$N默運神功﹐但覺真氣如玄風裂脈﹐飄逸之中自有一股護體的罡正。\n",
});
