...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("chaos steps");
    setup();
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("chaos steps");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺腳下步伐變得飄忽不定﹐亂步已經練成了﹗\n" NOR);
            me->advance_skill("chaos steps", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("chaos steps", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("dex") < level / 3 ) {
        tell_object(me, HIW "由於你亂步的修練有成﹐你的敏捷提高了。\n" NOR);
        me->add("dex", 1);
    }
}
