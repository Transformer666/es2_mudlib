...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("fonxanforce", 1);
    learn = me->query_learn("fonxanforce", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐封山內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("fonxanforce", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("fonxanforce", 1);
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("int") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        ...
        me->improve_skill("fonxanforce", random(me->query_attr("int")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "fonxanforce":
        message_vision(HIY "$N盤膝而坐﹐運起封山派的內功心法。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("封山內功沒有這種功能。\n");
    }
}
