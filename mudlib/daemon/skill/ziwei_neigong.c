...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("ziwei neigong");
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("ziwei neigong", 1);
    learn = me->query_learn("ziwei neigong", 1);

    if( !skill ) {
	if( learn >= 1500 ) {
	    tell_object(me,
		HIY "你感覺周身紫氣流轉﹐氣沉丹田﹐紫薇心經的內功已經練成了﹗\n" NOR);
	    me->advance_skill("ziwei neigong", 40);
	...
	me->advance_skill("ziwei neigong", 1);
...
int do_exercise(object me)
{
	...
	me->improve_skill("ziwei neigong", random(me->query_attr("spi")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "ziwei neigong":
	message_vision(HIY "$N盤膝而坐﹐深深地吸了口氣﹐開始修習紫薇心經的內功。\n" NOR,
	    me);
	me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
	return 1;
    default:
	return notify_fail("紫薇心經沒有這種功能。\n");
    }
}
