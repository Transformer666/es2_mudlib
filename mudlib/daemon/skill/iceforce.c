...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("iceforce");
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("iceforce", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐寒冰內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("iceforce", 40);
...
void skill_advanced(object me, string sk)
{
    ...
    if( me->query_stat_maximum("kee") < level * 12 )
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("spi") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        ...
        me->improve_skill("iceforce", random(me->query_attr("spi")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "iceforce":
        message_vision(HIY "$N盤膝而坐﹐運起寒冰真氣﹐周身寒氣瀰漫。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("寒冰內功沒有這種功能。\n");
    }
}
