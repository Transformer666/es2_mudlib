...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("maoshan neigong");
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("maoshan neigong", 1);
    learn = me->query_learn("maoshan neigong", 1);

    if( !skill ) {
	if( learn >= 1500 ) {
	    tell_object(me,
		HIY "你感覺體內真氣流轉不息﹐茅山心經的內功已經練成了﹗\n" NOR);
	    me->advance_skill("maoshan neigong", 40);
	...
	me->advance_skill("maoshan neigong", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( me->query_stat_maximum("gin") < level * 15 )
	me->advance_stat("gin", 6);
    if( me->query_stat_maximum("kee") < level * 13 )
	me->advance_stat("kee", 5);
    if( me->query_stat_maximum("sen") < level * 15 )
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("cps") + me->query_attr("spi") + me->query_skill("force", 1)) > 20 ) {
	me->damage_stat("gin", 1);
	me->damage_stat("kee", 1);
	me->damage_stat("sen", 1);
	me->improve_skill("maoshan neigong", random(me->query_attr("spi")/10) + 1);
...
exert_function(object me, string func, object target)
{
	...
	message_vision(HIY "$N盤膝而坐﹐運起茅山心經的內功心法。\n" NOR,
	    me);
	me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
	return 1;
    default:
	return notify_fail("茅山心經沒有這種功能。\n");
    }
}
