...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("essencemagic");
    setup();
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("essencemagic");

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺你感覺自己對精魄之術的領悟更深了一層﹗\n" NOR);
            me->advance_skill("essencemagic", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("essencemagic", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( level % 10 == 9 && me->query_attr("spi") < level / 3 ) {
        tell_object(me, HIW "由於你精魄法術的修練有成﹐你的靈力提高了。\n" NOR);
        me->add("spi", 1);
    }
}
