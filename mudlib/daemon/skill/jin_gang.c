...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("jin gang");
}
...
void skill_improved(object me, string sk)
{
    ...
    learn = me->query_learn("jin gang", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐金剛內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("jin gang", 40);
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("con") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        me->damage_stat("gin", 1);
        me->damage_stat("kee", 1);
        me->damage_stat("sen", 1);
        me->improve_skill("jin gang", random(me->query_attr("con")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "jin gang":
        message_vision(HIY "$N盤膝而坐﹐運起金剛內功﹐周身散發出金光。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("金剛內功沒有這種功能。\n");
    }
}
