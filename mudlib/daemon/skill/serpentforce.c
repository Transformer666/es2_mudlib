...
{
    seteuid(getuid());
    DAEMON_D->register_skill_daemon("serpentforce");
}
...
void skill_improved(object me, string sk)
{
    ...
    skill = me->query_skill("serpentforce", 1);
    learn = me->query_learn("serpentforce", 1);

    if( !skill ) {
        if( learn >= 1500 ) {
            tell_object(me,
                HIY "你感覺體內氣息調和﹐蛇力內功的內功已經練成了﹗\n" NOR);
            me->advance_skill("serpentforce", 40);
    ...
    if( (skill < 120) && (skill-39) * (skill-39) * 200 < learn - 1500 ) {
        me->advance_skill("serpentforce", 1);
...
void skill_advanced(object me, string sk)
{
    ...
    if( me->query_stat_maximum("kee") < level * 16 )
        me->advance_stat("kee", 5);
    if( me->query_stat_maximum("sen") < level * 14 )
...
int do_exercise(object me)
{
    ...
    if( random(me->query_attr("dex") + me->query_attr("wis") + me->query_skill("force", 1)) > 20 ) {
        me->damage_stat("gin", 1);
        me->damage_stat("kee", 1);
        me->damage_stat("sen", 1);
        me->improve_skill("serpentforce", random(me->query_attr("dex")/10) + 1);
...
exert_function(object me, string func, object target)
{
    switch(func) {
    case "serpentforce":
        message_vision(HIY "$N盤膝而坐﹐運起蛇力內功﹐身形如蛇般柔韌。\n" NOR,
            me);
        me->start_busy((: do_exercise, me :), (: halt_exercise, me :));
        return 1;
    default:
        return notify_fail("蛇力內功沒有這種功能。\n");
    }
}
